...
from smart_labelimg.ai_backend import ClassicalVisionBackend, OnnxMobileSamBackend, SamClickBackend
...
from smart_labelimg.image_io import read_image_bgr
...
MOBILE_SAM_ENCODER = resource_path("models/mobile_sam_encoder.onnx")
MOBILE_SAM_DECODER = resource_path("models/mobile_sam_decoder.onnx")
...
class MainWindow(QMainWindow):
    ...
    def _create_backend(self):
        if MOBILE_SAM_ENCODER.exists() and MOBILE_SAM_DECODER.exists():
            try:
                return OnnxMobileSamBackend(str(MOBILE_SAM_ENCODER), str(MOBILE_SAM_DECODER))
            except Exception as exc:
                print(f"ONNX MobileSAM unavailable, trying legacy backend: {exc}", file=sys.stderr)
    ...
    def _build_ui(self) -> None:
        ...
        hidden_shortcuts = [
            ...
            ("Save", self.save_current_explicit, QKeySequence.StandardKey.Save),
    ...
    def annotation_changed(self) -> None:
    ...
    def open_image_path(self, path: Path) -> None:
        if path.is_dir():
            candidates = sorted(child for child in path.iterdir() if child.suffix.lower() in IMAGE_SUFFIXES)
            if not candidates:
                self.status.showMessage("No supported images found in the selected folder")
                return
            stem_counts = Counter(child.stem.casefold() for child in candidates)
            duplicate_stems = sorted(stem for stem, count in stem_counts.items() if count > 1)
            if duplicate_stems:
                self.status.showMessage(
                    "Folder has images with the same base name; rename them before labeling: "
                    + ", ".join(duplicate_stems[:5])
                )
                return
        else:
            if not path.is_file() or path.suffix.lower() not in IMAGE_SUFFIXES:
                self.status.showMessage("Select a supported JPG, JPEG, PNG, BMP, or WebP image")
                return
            candidates = [path]
    ...
    def save_current(self, show_status: bool = True, force: bool = True) -> bool:
        ...
        if label_path is None:
            return True
        if not force and not self.annotation_dirty:
            return True
    ...
    def auto_save_current(self) -> None:
        ...
        if self.annotation_dirty:
            self.save_current(show_status=False, force=False)
