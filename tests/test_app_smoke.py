...
def test_image_folder_populates_file_list_and_selects_by_row(tmp_path):
    ...
    assert not (image_folder / "one.txt").exists()
    assert not (image_folder / "two.txt").exists()
    window.close()


def test_image_without_label_creates_yolo_files_after_first_edit(tmp_path):
    ...
    assert (tmp_path / "new.txt").read_text(encoding="utf-8").startswith("3 ")
    assert (tmp_path / "classes.txt").exists()
    window.close()


def test_image_in_unicode_windows_path_opens_and_saves_label(tmp_path):
    folder = tmp_path / "微信图片"
    folder.mkdir()
    image_path = folder / "测试图片.jpg"
    ...
    assert window.current_image == image_path
    assert window.image_size == (120, 80)
    assert (folder / "测试图片.txt").exists()
    window.close()


def test_explicit_save_can_mark_image_as_reviewed_with_empty_label(tmp_path):
    ...
    window.open_image_path(image_path)
    assert window.save_current_explicit()

    assert (tmp_path / "negative.txt").read_text(encoding="utf-8") == ""
    assert (tmp_path / "classes.txt").exists()
    window.close()


def test_empty_folder_does_not_discard_current_image(tmp_path):
    ...
    window.open_image_path(empty_folder)

    assert window.current_image == image_path
    assert "No supported images" in window.status.currentMessage()
    window.close()


def test_folder_with_duplicate_image_stems_is_rejected_to_prevent_label_collision(tmp_path):
    folder = tmp_path / "images"
    folder.mkdir()
    cv2.imwrite(str(folder / "same.jpg"), np.zeros((80, 120, 3), dtype=np.uint8))
    cv2.imwrite(str(folder / "same.png"), np.zeros((80, 120, 3), dtype=np.uint8))
    app = QApplication.instance() or QApplication([])
    window = MainWindow()

    window.open_image_path(folder)

    assert window.current_image is None
    assert "same base name" in window.status.currentMessage()
    window.close()
