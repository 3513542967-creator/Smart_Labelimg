...
from smart_labelimg.ai_backend import (
...
def fake_torch(cuda: bool, mps: bool = False):
...
def test_compute_device_uses_cuda_only_when_available(monkeypatch):
    monkeypatch.delenv("SMART_LABELIMG_DEVICE", raising=False)
    assert choose_compute_device(fake_torch(cuda=True)) == "cuda"
    assert choose_compute_device(fake_torch(cuda=False)) == "cpu"


def test_compute_device_can_be_forced_to_cpu(monkeypatch):
    monkeypatch.setenv("SMART_LABELIMG_DEVICE", "cpu")
    assert choose_compute_device(fake_torch(cuda=True)) == "cpu"


def test_invalid_device_override_falls_back_to_auto(monkeypatch):
    monkeypatch.setenv("SMART_LABELIMG_DEVICE", "not-a-device")
    assert choose_compute_device(fake_torch(cuda=False)) == "cpu"
